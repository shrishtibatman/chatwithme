...
async function initDB(db) {
  await db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_pending ON pending_replies(send_at, sent);
  `);
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && path === '/api/user') {
      ...
      await db.prepare(`
        INSERT INTO users (id, name, ig_handle, pw_hash, status, trusted, blocked, ignore_mode, warn_count, roast_mode, notes, last_seen)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).bind(
        b.id, b.name || '', b.ig_handle || null, b.pw_hash || null,
        b.status || 'normal', b.trusted || 0, b.blocked || 0,
        b.ignore_mode || 0, b.warn_count || 0, b.roast_mode || 0,
        b.notes || null, Date.now()
      ).run();
      return json({ ok: true });
    }

    // POST /api/user/:id/status — block/warn/ignore/trust/roast
    ...
    // GET /api/users — all users list for owner
    if (request.method === 'GET' && path === '/api/users') {
      const { results } = await db.prepare(
        `SELECT id, name, ig_handle, status, trusted, blocked, ignore_mode, warn_count, roast_mode, notes, last_seen
    ...
    if (request.method === 'GET' && path.startsWith('/api/chats/') && !path.includes('/pending')) {
      ...
      const { results } = await db.prepare(
        `SELECT role, content, ts, audio_id FROM conversations
         WHERE user_id = ? ORDER BY ts ASC LIMIT 300`
      ).bind(userId).all();
      return json(results || []);
    }

    // POST /api/chats/:userId — save one message
    if (request.method === 'POST' && path.startsWith('/api/chats/') && !path.includes('/pending')) {
      ...
      await db.prepare(
        `INSERT OR REPLACE INTO conversations (id, user_id, role, content, ts, audio_id)
         VALUES (?, ?, ?, ?, ?, ?)`
    ...
    // ════════════════════════════════
    // PENDING REPLIES (human timing)
    // ════════════════════════════════

    // POST /api/pending — queue a reply
    if (request.method === 'POST' && path === '/api/pending') {
      const b = await request.json();
      const id = 'p_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6);
      await db.prepare(
        `INSERT INTO pending_replies (id, user_id, reply, send_at, sent) VALUES (?, ?, ?, ?, 0)`
      ).bind(id, b.user_id, b.reply, b.send_at).run();
      return json({ ok: true, id });
    }

    // GET /api/pending/:userId — get pending reply for user (if time has come)
    if (request.method === 'GET' && path.startsWith('/api/pending/')) {
      const userId = decodeURIComponent(path.split('/api/pending/')[1]);
      const now = Date.now();
      const row = await db.prepare(
        `SELECT * FROM pending_replies WHERE user_id=? AND sent=0 AND send_at<=? ORDER BY send_at ASC LIMIT 1`
      ).bind(userId, now).first();
      if (row) {
        await db.prepare(`UPDATE pending_replies SET sent=1 WHERE id=?`).bind(row.id).run();
        return json({ reply: row.reply, id: row.id });
      }
      // Check if there's a future pending reply
      const future = await db.prepare(
        `SELECT send_at FROM pending_replies WHERE user_id=? AND sent=0 ORDER BY send_at ASC LIMIT 1`
      ).bind(userId).first();
    ...
    if (request.method === 'GET' && path === '/api/stats') {
      ...
      const pending = await db.prepare(`SELECT COUNT(*) as c FROM pending_replies WHERE sent=0`).first();
      return json({ users: users.c, messages: msgs.c, blocked: blocked.c, pending: pending.c });
    }

    // GET /api/all-convos — owner view
    if (request.method === 'GET' && path === '/api/all-convos') {
      const limit = parseInt(url.searchParams.get('limit') || '200');
      const { results } = await db.prepare(
        `SELECT c.user_id, c.role, c.content, c.ts, u.name, u.ig_handle
